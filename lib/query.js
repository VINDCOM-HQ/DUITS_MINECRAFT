...
function makeSessionId() {
  // generate a 32-bit signed integer, then mask to low 4 bits per byte
...
class QueryClient {
  ...
  async queryBasic() {
    ...
    console.log(`[Minecraft Query] basic -> ${this.host}:${this.port}`);
  ...
  async queryFull() {
    this.host = this.host.trim();
    await this._resolveSrv();
    console.log(`[Minecraft Query] full -> ${this.host}:${this.port}`);
    const challenge = await this._handshake();
