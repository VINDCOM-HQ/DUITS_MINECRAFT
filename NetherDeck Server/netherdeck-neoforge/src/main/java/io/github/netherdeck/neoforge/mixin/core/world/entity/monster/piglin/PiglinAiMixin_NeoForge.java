package io.github.netherdeck.neoforge.mixin.core.world.entity.monster.piglin;
...
public abstract class PiglinAiMixin_NeoForge {
    ...
    private static boolean netherdeck$customBarter(ItemStack stack, Piglin piglin) {
    ...
    private static boolean netherdeck$customBanter2(ItemStack stack, Piglin piglin) {
        return isBarterCurrency(stack) || ((PiglinBridge) piglin).bridge$getAllowedBarterItems().contains(stack.getItem());
    }

    @Redirect(method = "canAdmire", at = @At(value = "INVOKE", remap = false, target = "Lnet/minecraft/world/item/ItemStack;isPiglinCurrency()Z"))
    private static boolean netherdeck$customBanter3(ItemStack stack, Piglin piglin) {
        return isBarterCurrency(stack) || ((PiglinBridge) piglin).bridge$getAllowedBarterItems().contains(stack.getItem());
    }
}
