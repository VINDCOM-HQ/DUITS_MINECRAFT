...
abstract class UploadFilesTask extends DefaultTask {
    ...
    @TaskAction
    void run() {
        if (!System.getenv().NETHERDECK_FILES_TOKEN) {
            project.logger.lifecycle('No netherdeck token set, skipping upload files.')
            return
        }
    ...
    private void uploadOne(File file) {
        ...
        (new URL(OBJECTS.formatted(sha1)).openConnection() as HttpURLConnection).with {
            ...
            it.addRequestProperty("AuthToken", System.getenv().NETHERDECK_FILES_TOKEN)
        ...
        link("/netherdeck/branches/${branch.get()}/versions-snapshot/${version.get()}/${modloader}", [type: 'object', value: sha1])
        link("/netherdeck/branches/${branch.get()}/loaders/${modloader}/versions-snapshot/${version.get()}", [type: 'object', value: sha1])
        link("/netherdeck/branches/${branch.get()}/latest-snapshot", [type: 'link', value: "/netherdeck/branches/${branch.get()}/versions-snapshot/${version.get()}", cache_seconds: 3600])
        link("/netherdeck/branches/${branch.get()}/loaders/${modloader}/latest-snapshot", [type: 'link', value: "/netherdeck/branches/${branch.get()}/loaders/${modloader}/versions-snapshot/${version.get()}", cache_seconds: 3600])
        if (!snapshot.get()) {
            link("/netherdeck/branches/${branch.get()}/versions-stable/${version.get()}/${modloader}", [type: 'object', value: sha1])
            link("/netherdeck/branches/${branch.get()}/loaders/${modloader}/versions-stable/${version.get()}", [type: 'object', value: sha1])
            link("/netherdeck/branches/${branch.get()}/latest-stable", [type: 'link', value: "/netherdeck/branches/${branch.get()}/versions-stable/${version.get()}", cache_seconds: 86400])
            link("/netherdeck/branches/${branch.get()}/loaders/${modloader}/latest-stable", [type: 'link', value: "/netherdeck/branches/${branch.get()}/loaders/${modloader}/versions-stable/${version.get()}", cache_seconds: 86400])
        }
    }
