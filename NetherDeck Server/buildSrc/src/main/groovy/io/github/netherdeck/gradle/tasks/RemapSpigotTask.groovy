...
class RemapSpigotTask implements Runnable {
    ...
    @Override
    void run() {
        def tmp = Files.createTempFile("netherdeck", "jar")
        ...
        if (inExtraSrg) {
            def tmp2 = Files.createTempFile("netherdeck", "jar")
            copy(tmp, tmp2, ['*'], [
        ...
        def tmpDeobf = Files.createTempFile("netherdeck", "jar")
        def args = [
        ...
        Path tmpSrg
        if (bukkitVersion) {
            tmpSrg = Files.createTempFile("netherdeck", "srg")
