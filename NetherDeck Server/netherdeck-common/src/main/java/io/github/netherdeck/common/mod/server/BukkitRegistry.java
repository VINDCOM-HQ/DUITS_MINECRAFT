...
import io.github.netherdeck.common.mod.server.entity.EntityClassLookup;
import io.github.netherdeck.common.mod.util.ResourceLocationUtil;
import io.github.netherdeck.i18n.NetherDeckConfig;
import io.github.netherdeck.i18n.conf.EntityPropertySpec;
...
public class BukkitRegistry {
    ...
    private static void loadGameRules() {
        ...
        try {
            ...
            NetherDeckServer.LOGGER.warn("Cannot register all custom game rules for bukkit!", e);
            NetherDeckServer.LOGGER.warn("This is a bug, and will cause commands like mvgamerule not working properly. Please report this!");
            return;
        }
            ...
            public <T extends GameRules.Value<T>> void visit(GameRules.Key<T> key, GameRules.Type<T> type) {
                if (!gameRules.containsKey(key.getId())) {
                    ...
                    try {
                        ...
                        NetherDeckServer.LOGGER.warn("Cannot register custom game rule {} for bukkit!", key.getId(), e);
                    }
                }
            }
        });
    ...
    private static void loadFluids() {
        ...
        for (var fluidType : BuiltInRegistries.FLUID) {
            ...
            try {
                ...
                newTypes.add(bukkit);
                NetherDeckServer.LOGGER.debug("Registered {} as fluid {}", key, bukkit);
            }
        }
    ...
    private static void loadCraftingBookCategory() {
        ...
        for (CraftingBookCategory category : CraftingBookCategory.values()) {
            try {
                ...
                newTypes.add(bukkit);
                NetherDeckServer.LOGGER.debug("Registered {} as crafting category {}", name, bukkit);
            }
        }
    ...
    private static void loadCookingBookCategory() {
        ...
        for (CookingBookCategory category : CookingBookCategory.values()) {
            try {
                ...
                newTypes.add(bukkit);
                NetherDeckServer.LOGGER.debug("Registered {} as cooking category {}", name, bukkit);
            }
        }
    ...
    private static void loadRecipeBookType() {
        ...
        for (RecipeBookType type : RecipeBookType.values()) {
            ...
            if (!knownTypes.remove(name)) {
                ...
                newTypes.add(bukkit);
                NetherDeckServer.LOGGER.debug("Registered {} as recipe book type {}", name, bukkit);
            }
        }
        if (!knownTypes.isEmpty()) {
            NetherDeckServer.LOGGER.fatal("Assertion failed, found unknown recipe book type in Bukkit: {}", knownTypes);
    ...
    private static void loadEndDragonPhase() {
        ...
        for (var id = EnderDragon.Phase.values().length; id < max; id++) {
            ...
            NetherDeckServer.LOGGER.debug("Registered {} as ender dragon phase {}", name, newPhase);
        }
    ...
    private static void loadSpawnCategory() {
        ...
        for (var category : MobCategory.values()) {
            try {
                ...
                NetherDeckServer.LOGGER.debug("Registered {} as spawn category {}", name, spawnCategory);
            }
        }
    ...
    private static void loadStats() {
        ...
        for (StatType<?> statType : BuiltInRegistries.STAT_TYPE) {
            ...
            if (statistic == null) {
                ...
                NetherDeckServer.LOGGER.debug("Registered {} as stats {}", location, statistic);
                i++;
            }
        }
        for (ResourceLocation location : BuiltInRegistries.CUSTOM_STAT) {
            ...
            if (statistic == null) {
                ...
                newTypes.add(statistic);
                STATS.put(location, statistic);
                NetherDeckServer.LOGGER.debug("Registered {} as custom stats {}", location, statistic);
                i++;
            }
        }
    ...
    private static void loadArts(DedicatedServer console) {
        ...
        for (var paintingType : reg) {
            ...
            if (bukkit == null) {
                ...
                NetherDeckServer.LOGGER.debug("Registered {} as art {}", location, bukkit);
                i++;
            }
        }
    ...
    private static void loadBiomes(DedicatedServer console) {
        ...
        for (net.minecraft.world.level.biome.Biome biome : registry) {
            ...
            if (bukkit == null) {
                ...
                newTypes.add(bukkit);
                Unsafe.putObject(bukkit, keyOffset, CraftNamespacedKey.fromMinecraft(location));
                NetherDeckServer.LOGGER.debug("Registered {} as biome {}", location, bukkit);
            }
        }
        EnumHelper.addEnums(Biome.class, newTypes);
        NetherDeckServer.LOGGER.info("registry.biome", newTypes.size());
    }
    ...
    public static void registerEnvironments(Registry<LevelStem> registry) {
        ...
        for (Map.Entry<ResourceKey<LevelStem>, LevelStem> entry : registry.entrySet()) {
            ...
            if (environment == null) {
                ...
                NetherDeckServer.LOGGER.debug("Registered {} as environment {}", key.location(), environment);
                i++;
            }
        }
        EnumHelper.addEnums(World.Environment.class, newTypes);
        NetherDeckServer.LOGGER.info("registry.environment", newTypes.size());
    }
    ...
    private static void loadEntities() {
        ...
        for (net.minecraft.world.entity.EntityType<?> type : BuiltInRegistries.ENTITY_TYPE) {
            ...
            if (location.getNamespace().equals(NamespacedKey.MINECRAFT)) {
                ...
                else NetherDeckServer.LOGGER.warn("Not found {} in {}", location, EntityType.class);
            }
            if (!found) {
                String name = ResourceLocationUtil.standardize(location);
                ...
                NetherDeckServer.LOGGER.debug("Registered {} as entity {}", location, entityType);
            }
            ENTITY_NAME_MAP.put(location.toString(), entityType);
        }
        EnumHelper.addEnums(EntityType.class, newTypes);
        EntityClassLookup.init();
        NetherDeckServer.LOGGER.info("registry.entity-type", newTypes.size());
    }
    ...
    private static void loadPotions() {
        ...
        for (var potion : BuiltInRegistries.POTION) {
            ...
            try {
                ...
                NetherDeckServer.LOGGER.debug("Registered {} as potion type {}", location, potionType);
            }
        }
    ...
    private static void loadMaterials() {
        ...
        for (Block block : BuiltInRegistries.BLOCK) {
            ...
            if (material == null) {
                ...
                i++;
                blocks++;
                NetherDeckServer.LOGGER.debug("Registered {} as block {}", location, material);
                list.add(material);
            } else {
        ...
        for (Item item : BuiltInRegistries.ITEM) {
            ...
            if (material == null) {
                ...
                i++;
                items++;
                NetherDeckServer.LOGGER.debug("Registered {} as item {}", location, material);
                list.add(material);
            }
        ...
        NetherDeckServer.LOGGER.info("registry.material", i - origin, blocks, items);
    }

    private static MaterialPropertySpec matSpec(ResourceLocation location) {
        return NetherDeckConfig.spec().getCompat().getMaterial(location.toString()).orElse(MaterialPropertySpec.EMPTY);
    }

    private static EntityPropertySpec entitySpec(ResourceLocation location) {
        return NetherDeckConfig.spec().getCompat().getEntity(location.toString()).orElse(EntityPropertySpec.EMPTY);
    }
    ...
    public static Pose toBukkitPose(net.minecraft.world.entity.Pose nms) {
        if (Pose.values().length <= nms.ordinal()) {
            ...
            for (var id = Pose.values().length; id < forgeCount; id++) {
                ...
                newTypes.add(newPhase);
                NetherDeckServer.LOGGER.debug("Registered {} as pose {}", name, newPhase);
            }
