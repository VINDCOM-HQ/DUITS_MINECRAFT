...
public class DistValidate {
    ...
    private static boolean isLogicWorld(Class<?> cl) {
        return SEEN_CLASSES.computeIfAbsent(cl, c -> {
            var name = c.getName();
            var result = NetherDeckConfig.spec().getCompat().getExtraLogicWorlds().contains(cl.getName());
            NetherDeckServer.LOGGER.warn(MARKER, "Level class {} treated as logic world: {}", name, result);
            return result;
        });
    }
}
