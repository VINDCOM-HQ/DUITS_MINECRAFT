...
public class ClassLoaderAdapter implements PluginTransformer {
    ...
    public void handleClass(ClassNode node, ClassLoaderRemapper remapper, NetherDeckRemapConfig config) {
            ...
            for (AbstractInsnNode insnNode : methodNode.instructions) {
                if (insnNode.getOpcode() == Opcodes.NEW) {
                    ...
                    if (replace != null) {
                        ...
                        NetherDeckServer.LOGGER.debug(MARKER, "Found new {}/{} call in {} {}", typeInsnNode.desc, ((MethodInsnNode) next).name + ((MethodInsnNode) next).desc, node.name, methodNode.name + methodNode.desc);
        ...
        if (info == null) return;
        NetherDeckServer.LOGGER.debug(MARKER, "Transforming classloader class {}", node.name);
    ...
    private void implementIntf(ClassNode node) {
        NetherDeckServer.LOGGER.debug(MARKER, "Implementing RemappingClassLoader for class {}", node.name);
        ...
        MethodNode getRemapper = new MethodNode(Opcodes.ACC_PUBLIC | Opcodes.ACC_SYNTHETIC, "getRemapper", Type.getMethodDescriptor(Type.getType(ClassLoaderRemapper.class)), null, null);
        {
            ...
            list.add(new MethodInsnNode(Opcodes.INVOKESTATIC, Type.getInternalName(NetherDeckRemapper.class), "createClassLoaderRemapper", Type.getMethodDescriptor(Type.getType(ClassLoaderRemapper.class), Type.getType(ClassLoader.class)), false));
        ...
        FieldNode remapConfig = new FieldNode(Opcodes.ACC_PRIVATE | Opcodes.ACC_SYNTHETIC, "netherdeck$remapConfig", Type.getDescriptor(NetherDeckRemapConfig.class), null, null);
        MethodNode getConfig = new MethodNode(Opcodes.ACC_PUBLIC | Opcodes.ACC_SYNTHETIC, "getRemapConfig", Type.getMethodDescriptor(Type.getType(NetherDeckRemapConfig.class)), null, null);
        {
            final var config = Type.getInternalName(NetherDeckRemapConfig.class);
            final var remapping = Type.getInternalName(ClassLoaderRemapping.class);

            /*
             * public NetherDeckRemapConfig getConfig() {
             *     if (netherdeck$remapConfig != null) {
             *         return netherdeck$remapConfig;
             *     }
             *     return netherdeck$remapConfig = new NetherDeckRemapConfig(RemappingClassLoader.needRemap(this));
             * }
             */
            InsnList list = new InsnList();
