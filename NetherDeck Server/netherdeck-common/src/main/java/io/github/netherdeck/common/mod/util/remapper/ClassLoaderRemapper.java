...
import io.github.netherdeck.common.mod.util.remapper.generated.NetherDeckReflectionHandler;
...
public class ClassLoaderRemapper extends LenientJarRemapper {
    ...
    private static final String REPLACED_NAME = Type.getInternalName(NetherDeckReflectionHandler.class);
    ...
    public ClassLoaderRemapper(JarMapping jarMapping, JarMapping toBukkitMapping, ClassLoader classLoader) {
        ...
        this.jarMapping.setInheritanceMap(NetherDeckRemapper.INSTANCE.inheritanceMap);
    ...
    public Map.Entry<String, String> tryClimb(Map<String, String> map, String owner, String name, int access) {
        ...
        if (mapped == null && (access == -1 || (!Modifier.isPrivate(access) && !Modifier.isStatic(access)))) {
            ...
            if (NetherDeckRemapper.INSTANCE.inheritanceMap.hasParents(owner)) {
                parents = NetherDeckRemapper.INSTANCE.inheritanceMap.getParents(owner);
            } else {
                parents = GlobalClassRepo.inheritanceProvider().getParents(owner);
                NetherDeckRemapper.INSTANCE.inheritanceMap.setParents(owner, parents);
            }
    ...
    public Product2<byte[], CodeSource> remapClass(String className, Callable<byte[]> byteSource, URLConnection connection, NetherDeckRemapConfig config) throws ClassNotFoundException {
        try {
            NetherDeckClassCache.CacheSegment segment = NetherDeckClassCache.instance().makeSegment(connection);
            ...
            if (optional.isPresent()) {
                ...
                if (NetherDeckConfig.spec().getOptimization().isCachePluginClass()) {
                    ClassWriter cw = new ClassWriter(0);
    ...
    private byte[] remapClassFile(ClassReader reader, final ClassRepo repo) {
        ...
        NetherDeckRemapConfig config;
        if (repo instanceof ClassRepoWrapper wrapper) {
            config = wrapper.config();
        } else {
            NetherDeckServer.LOGGER.warn("No class remap config is provided for class {}, using PLUGIN", node.name.replace('/','.'));
            config = NetherDeckRemapConfig.PLUGIN;
        }

        for (PluginTransformer transformer : NetherDeckRemapper.INSTANCE.getTransformerList()) {
    ...
    private Class<?> generateReflectionHandler() {
        try {
            ClassNode node = MixinService.getService().getBytecodeProvider().getClassNode(Type.getInternalName(NetherDeckReflectionHandler.class));
            Preconditions.checkNotNull(node, "node");
            ClassWriter writer = new ClassWriter(0);
            String name = Type.getInternalName(NetherDeckReflectionHandler.class) + "_" + COUNTER.getAndIncrement();
    ...
    private static class NameRemapper extends Remapper {
        ...
        private static final String ORIGIN = Type.getInternalName(NetherDeckReflectionHandler.class);
    ...
    private static class PluginClassWriter extends ClassWriter {
        ...
        private String getSuper(final String typeName) {
            ...
            return NetherDeckRemapper.getNmsMapper().map(node.superName);
        }
    }
    ...
    private static byte[] dump(byte[] bytes) {
        try {
            if (NetherDeckRemapper.DUMP != null) {
                String className = new ClassReader(bytes).getClassName() + ".class";
                int index = className.lastIndexOf('/');
                if (index != -1) {
                    File file = new File(NetherDeckRemapper.DUMP, className.substring(0, index));
                    file.mkdirs();
                    Files.write(file.toPath().resolve(className.substring(index + 1)), bytes);
                } else {
                    Files.write(NetherDeckRemapper.DUMP.toPath().resolve(className), bytes);
                }
            }
        } catch (Exception e) {
