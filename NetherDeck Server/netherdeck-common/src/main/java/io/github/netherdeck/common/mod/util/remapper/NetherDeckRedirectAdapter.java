...
import io.github.netherdeck.common.util.ArrayUtil;
...
public class NetherDeckRedirectAdapter implements PluginTransformer {

    public static final NetherDeckRedirectAdapter INSTANCE = new NetherDeckRedirectAdapter();
    ...
    public static void scanMethod(byte[] bytes) {
        ...
        NetherDeckServer.LOGGER.debug(MARKER, "Scanning {}", reader.getClassName());
                    ...
                    if (METHOD_MODIFY.containsKey(key) || METHOD_REDIRECT.containsKey(key)) {
                        try {
                            ...
                            if (target != null) {
                                ...
                                if (bridge != null) {
                                    NetherDeckServer.LOGGER.debug(MARKER, "Creating bridge handler {}/{}{} to {}", node.name, method.name, method.desc, methodToString(target));
    ...
    private static void addRule(boolean modifyArgs, Class<?> owner, String name, String handlerName, Class<?>... args) {
        ...
        Method handler = methodOf(NetherDeckReflectionHandler.class, "redirect" + capitalize(handlerName), handlerArgs);
        while (handler == null) {
            handlerArgs[0] = handlerArgs[0].getSuperclass();
            handler = methodOf(NetherDeckReflectionHandler.class, "redirect" + capitalize(handlerName), handlerArgs);
        }
        ...
        if (modifyArgs) {
            Method modifyHandler = methodOf(NetherDeckReflectionHandler.class, "handle" + capitalize(handlerName), handlerArgs);
            if (modifyHandler == null) {
                handlerArgs[0] = original.getReturnType();
                modifyHandler = methodOf(NetherDeckReflectionHandler.class, "handle" + capitalize(handlerName), handlerArgs);
            }
            if (modifyHandler == null) {
