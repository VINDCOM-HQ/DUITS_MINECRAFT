...
import io.github.netherdeck.i18n.conf.AsyncCatcherSpec;
...
public class AsyncCatcher implements Implementer {
    ...
    public AsyncCatcher() {
        ...
        this.defaultOp = NetherDeckConfig.spec().getAsyncCatcher().getDefaultOp();
        this.dump = NetherDeckConfig.spec().getAsyncCatcher().isDump();
        this.warn = NetherDeckConfig.spec().getAsyncCatcher().isWarn();
    }
    ...
    private void injectCheck(ClassNode node, MethodNode methodNode, String reason) {
        ...
        AsyncCatcherSpec.Operation operation = NetherDeckConfig.spec().getAsyncCatcher().getOverrides().getOrDefault(reason, defaultOp);
        InsnList insnList = new InsnList();
        LabelNode labelNode = new LabelNode(new Label());
        LabelNode labelNode1 = new LabelNode(new Label());
        insnList.add(new MethodInsnNode(Opcodes.INVOKESTATIC, "io/github/netherdeck/common/mod/server/NetherDeckServer", "isPrimaryThread", "()Z"));
        ...
        insnList.add(new MethodInsnNode(Opcodes.INVOKESTATIC, "io/github/netherdeck/common/mod/server/NetherDeckServer", "getMainThreadExecutor", "()Ljava/util/concurrent/Executor;", false));
        insnList.add(new MethodInsnNode(Opcodes.INVOKESTATIC, Type.getType(AsyncCatcher.class).getInternalName(), "checkOp", "(Ljava/util/function/Supplier;Lio/github/netherdeck/i18n/conf/AsyncCatcherSpec$Operation;Ljava/lang/String;Ljava/util/concurrent/Executor;)Lorg/spongepowered/asm/mixin/injection/callback/CallbackInfoReturnable;"));
