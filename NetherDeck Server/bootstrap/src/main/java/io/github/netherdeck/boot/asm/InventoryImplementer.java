...
public class InventoryImplementer implements Implementer {
    ...
    private static final String BRIDGE_TYPE = "io/github/netherdeck/common/bridge/core/inventory/IInventoryBridge";

    private final String maxStackSizeMethodName;

    public InventoryImplementer() {
        this.maxStackSizeMethodName = switch (NetherDeckPlatform.current()) {
    ...
    private boolean tryImplement(ClassNode node) {
        ...
        if (stackLimitMethod == null) {
            ...
            FieldNode maxStack = new FieldNode(Opcodes.ACC_PRIVATE, "netherdeck$maxStack", Type.getType(Integer.class).getDescriptor(), null, null);
