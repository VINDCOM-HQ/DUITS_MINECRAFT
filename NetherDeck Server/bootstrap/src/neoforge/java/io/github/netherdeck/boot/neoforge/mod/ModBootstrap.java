...
import io.github.netherdeck.installer.ForgeInstaller;
import io.github.netherdeck.installer.MinecraftProvider;
...
public class ModBootstrap implements AbstractBootstrap {
    ...
    static void run() {
        var plugin = Launcher.INSTANCE.environment().findLaunchPlugin("netherdeck_implementer");
        if (plugin.isPresent()) return;
        var logger = LogManager.getLogger("NetherDeck");
        ...
        try {
            ...
            logger.error("Error bootstrap NetherDeck", e);
    ...
    private void inject() throws Throwable {
        dirtyHacks();
        setupMod(NetherDeckPlatform.NEOFORGE);
        injectClassPath();
        injectLaunchPlugin();
    }
    ...
    private void injectLaunchPlugin() throws Exception {
        ...
        var plugin = new NetherDeckImplementer();
