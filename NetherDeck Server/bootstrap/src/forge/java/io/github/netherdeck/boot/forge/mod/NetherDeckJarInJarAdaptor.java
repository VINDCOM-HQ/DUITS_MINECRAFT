...
public class NetherDeckJarInJarAdaptor implements IDependencyLocator {

    private static final Logger LOGGER = LoggerFactory.getLogger("NetherDeckJiJ");

    private final IDependencyLocator delegate;

    public NetherDeckJarInJarAdaptor(IDependencyLocator delegate) {
    ...
    static void inject() {
        try {
            ...
            var field = ModDiscoverer.class.getDeclaredField("netherdeck$INSTANCE");
            ...
            var newList = locatorList.stream().map(it -> {
                if (it instanceof JarInJarDependencyLocator) {
                    return new NetherDeckJarInJarAdaptor(it);
                } else {
                    return it;
                }
            }).toList();
