package io.github.netherdeck.boot.forge.application;
...
public class ApplicationBootstrap extends ForgeBootstrap {
    ...
    protected void bootstrapMain(String[] args, List<Path[]> classpath) {
        try {
            ...
            var netherdeck = classpath.stream().map(SecureJar::from).filter(it -> it.moduleDataProvider().name().equals("netherdeck.boot")).findAny().orElseThrow();
            var jar = SecureJar.from(it -> new SimpleJarMetadata("netherdeck.launch", "1.0", Set.of("io.github.netherdeck.boot.forge.application"), List.of()), netherdeck.getPrimaryPath());
            boot.add(jar);
            ...
            var bootstrap = layer.findModule("netherdeck.launch").orElseThrow();
    ...
    public void moduleMain(String[] args, List<Path[]> classpath) throws Exception {
        ...
        var secure = selectRuntimeModules(classpath).stream().filter(it -> !mergedModules.contains(it.moduleDataProvider().name()) && !it.moduleDataProvider().name().equals("netherdeck.boot"))
        ...
        var netherdeck = classpath.stream().map(SecureJar::from).filter(it -> it.moduleDataProvider().name().equals("netherdeck.boot")).findAny().orElseThrow();
        secure.add(SecureJar.from(it -> new SimpleJarMetadata(netherdeck.name(), netherdeck.moduleDataProvider().descriptor().rawVersion().orElse("1.0"),
            netherdeck.getPackages().stream().filter(p -> !p.equals("io.github.netherdeck.boot.forge.application")).collect(Collectors.toSet()), netherdeck.getProviders()), netherdeck.getPrimaryPath()));
        ...
        try {
            ...
            var services = ServiceLoader.load(layer, BootstrapEntryPoint.class).stream().filter(it -> it.type().getName().contains("netherdeck")).toList();
